...
class UIRenderer {
  ...
  createMergedRouteCard(mergedRoute, routeNumber) {
    ...
    const abbreviatedRoute = mergedRoute.path.map(node => this.abbrList[node] || node);
    const displayRoute = abbreviatedRoute.join(' → ');

    const jumps = mergedRoute.path.length;
    const daysRange = mergedRoute.totalDays === mergedRoute.maxDays 
    ...
    sourcesHTML += '<div class="merged-sources-header">Starting trackers:</div>';
    ...
    mergedRoute.sourceRoutes.forEach(sourceRoute => {
      ...
      if (this.routeInfo[firstNode] && this.routeInfo[firstNode][secondNode]) {
        ...
        const className = this.unlockInviteClass[firstNode] ? this.unlockInviteClass[firstNode][1] : 'N/A';
        ...
        sourcesHTML += `
          <div class="merged-source-item">
            ...
            <div class="merged-source-details">
              ...
              ${requirement && requirement.toLowerCase() !== 'none' && requirement.toLowerCase() !== 'no requirement' ? `
                <div class="route-step-detail">
