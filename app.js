...
window.addEventListener('load', async function () {
  ...
  const routeCalculator = new RouteCalculator(routeInfo, unlockInviteClass, allTrackers, getAbbrFn);
  const uiRenderer = new UIRenderer(routeInfo, unlockInviteClass, abbrList, getMaxDays);
  ...
  // Setup autocomplete
  autocomplete.setupEventListeners(sourceInput, sourceDropdown, targetInput, targetDropdown, onAutocompleteSelect);

  sourceInput.addEventListener('input', (e) => {
    clearTimeout(sourceDebounceTimer);
    const value = e.target.value;
    sourceDebounceTimer = setTimeout(() => {
      autocomplete.show(sourceInput, sourceDropdown, value, onAutocompleteSelect);
      handleInputActivity();
    }, 150);
  });
  ...
  targetInput.addEventListener('input', (e) => {
    ...
    targetDebounceTimer = setTimeout(() => {
      autocomplete.show(targetInput, targetDropdown, value, onAutocompleteSelect);
      handleInputActivity();
    }, 150);
  });

  sourceInput.addEventListener('focus', (e) => {
    autocomplete.show(sourceInput, sourceDropdown, e.target.value, onAutocompleteSelect);
    handleInputActivity();
  });

  targetInput.addEventListener('focus', (e) => {
    autocomplete.show(targetInput, targetDropdown, e.target.value, onAutocompleteSelect);
    handleInputActivity();
  });
